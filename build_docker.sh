...
docker start $NAME
docker exec -it $NAME /bin/bash -c "rustup default 1.69; rustup target add wasm32-unknown-unknown; cargo build -p fungible-token --target wasm32-unknown-unknown --release"
